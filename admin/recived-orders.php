...
$query="select *from orders_details where status='recived' and order_id LIKE '%".$searchid."%' order by oid desc LIMIT ".$pageLimit." , ".$setLimit;
$mysqluery=mysql_query($query);
}else{
$query="select *from orders_details where status='recived' order by oid desc LIMIT ".$pageLimit." , ".$setLimit;
$mysqluery=mysql_query($query);
...
<div class="col-sm-9 col-sm-offset-3 col-lg-10 col-lg-offset-2 main">			
        ...
        <img id="loading-image" src="images/show_loader.gif" alt="Loading..." />
			...
			<ol class="breadcrumb">
				...
				<li class="active">Received Process</li>
			</ol>
		</div><!--/.row-->
		
		<div class="row">
			<div class="col-lg-12">
				<h1 class="page-header">Received Orders</h1>
	...
	<div class="row">
		...
		<div class="panel panel-default">
			<div class="panel-heading">Order ID &nbsp;<a class="btn btn-primary" href="#"><?php echo $orders['order_id']; ?> </a>&nbsp;&nbsp;<a href="recived-orders.php" class="btn btn-danger">Back</a></div>
...
<input type="submit" class="btn btn-primary" value="Search" onclick="return search_result()" />
					  ...
					  <?php 
                        ...
                        <tr>
                          ...
                          <td><?php echo $row['date']; ?></td>
						   <td><button class="btn btn-success"  disabled title="<?php echo $row['status']; ?>">Received Orders</button>
						  </td>
                          <td>
						  ...
						  <a href="recived-orders.php?vieworders=opendata&catid=<?php echo $row['order_id']; ?>&searchkeyowords=<?php echo $_GET['searchkeyowords']; ?>"  class="btn btn-primary" title="Edit"><span class="glyphicon glyphicon-eye-open"></span></a>&nbsp;&nbsp;
						  <?php }else{ ?>
						    <a href="recived-orders.php?vieworders=opendata&catid=<?php echo $row['order_id']; ?>"  class="btn btn-primary" title="Edit"><span class="glyphicon glyphicon-eye-open"></span></a>&nbsp;&nbsp;
						  <?php } ?>
						</td>
                        </tr>
						<?php } }else{ ?>
		...
		<div class="row">
				<div class="col-md-12">
				<?php if(isset($_GET['searchkeyowords'])) { echo deliveredsearchorders($setLimit,$page,$_GET['searchkeyowords']); }else { echo recivedorders($setLimit,$page); } ?>
		</div>
			</div>
		</div>
		</div>
...
window.location='recived-orders.php?searchkeyowords='+search_id2;
return false;
}
