...
$query="select *from orders_details where status='send' and order_id LIKE '%".$searchid."%'  order by oid desc LIMIT ".$pageLimit." , ".$setLimit;
$mysqluery=mysql_query($query);
}else{
$query="select *from orders_details where status='send' order by oid desc LIMIT ".$pageLimit." , ".$setLimit;
$mysqluery=mysql_query($query);
}
?>
<style>
...
<div class="col-sm-9 col-sm-offset-3 col-lg-10 col-lg-offset-2 main">			
	...
	<div class="row">
		...
		<div class="panel panel-default">
			...
			<a href="new-orders.php?searchkeyowords=<?php echo $_GET['searchkeyowords']; ?>" class="btn btn-danger">Back</a>
			<?php }else{ ?>
				<a href="new-orders.php" class="btn btn-danger">Back</a>
			<?php } ?>
			</div>
...
<input type="submit" class="btn btn-primary" value="Search" onclick="return search_result()" />
					  ...
					  <?php 
                        ...
                        <tr>
                          ...
                          <td><?php echo $row['date']; ?></td>
						   <td>
						    <?php if(isset($_GET['searchkeyowords'])) { ?>
						  <a href="post.php?action=confirmorders&confirm=<?php echo $row['oid']; ?>&searchkeyowords=<?php echo $_GET['searchkeyowords']; ?>" onclick="return confirm('are you sure to this order delivery');"  class="btn btn-success" title="<?php echo $row['status']; ?>">Confirm Orders</a>
						  <?php }else{ ?>
						   <a href="post.php?action=confirmorders&confirm=<?php echo $row['oid']; ?>" onclick="return confirm('are you sure to this order delivery');"  class="btn btn-success" title="<?php echo $row['status']; ?>">Confirm Orders</a>
						  <?php } ?>
						  </td>
                          <td>
					<!--	  <a href="new-orders.php?vieworders=opendata&catid=<?php echo $row['oid']; ?>"  class="btn btn-primary" title="Edit"><span class="glyphicon glyphicon-eye-open"></span></a>&nbsp;&nbsp; -->
						
						  <?php if(isset($_GET['searchkeyowords'])) { ?>
						  <a href="new-orders.php?vieworders=opendata&catid=<?php echo $row['order_id']; ?>&searchkeyowords=<?php echo $_GET['searchkeyowords']; ?>"  class="btn btn-primary" title="Edit"><span class="glyphicon glyphicon-eye-open"></span></a>&nbsp;&nbsp;
						  <?php }else{ ?>
						    <a href="new-orders.php?vieworders=opendata&catid=<?php echo $row['order_id']; ?>"  class="btn btn-primary" title="Edit"><span class="glyphicon glyphicon-eye-open"></span></a>&nbsp;&nbsp;
						  <?php } ?>
						  <a href="post.php?action=deletedeliveryorders&delete=<?php echo $row['oid']; ?>" onclick="return confirm('are you sure to remove this orders !'); " class="btn btn-danger" title="Delete" ><span class="glyphicon glyphicon-trash"></span></a></td>
		...
		<div class="row">
				<div class="col-md-12">
				<?php if(isset($_GET['searchkeyowords'])) { echo ordersendssearchs($setLimit,$page,$_GET['searchkeyowords']); }else { echo ordersends($setLimit,$page); } ?>
		</div>
			</div>
		</div>
		</div>
...
window.location='new-orders.php?searchkeyowords='+search_id2;
return false;
}
