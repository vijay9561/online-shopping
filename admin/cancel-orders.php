...
$query='';
//$query="select *from orders_details where status='cancel' order by oid desc LIMIT ".$pageLimit." , ".$setLimit;
//$mysqluery=mysql_query($query);
if(isset($_GET['searchkeyowords'])){
$searchid=$_GET['searchkeyowords'];
$query="select *from orders_details where status='cancel' and order_id LIKE '%".$searchid."%' order by oid desc LIMIT ".$pageLimit." , ".$setLimit;
$mysqluery=mysql_query($query);
}else{
$query="select *from orders_details where status='cancel' order by oid desc LIMIT ".$pageLimit." , ".$setLimit;
$mysqluery=mysql_query($query);
...
<div class="col-sm-9 col-sm-offset-3 col-lg-10 col-lg-offset-2 main">			
		...
		<div class="row">
			<div class="col-lg-12">
				<h1 class="page-header">Cancel Orders</h1>
	...
	<div class="row">
		...
		<div class="panel panel-default">
			...
			<a href="cancel-orders.php?searchkeyowords=<?php echo $_GET['searchkeyowords']; ?>" class="btn btn-danger">Back</a>
			<?php }else{ ?>
				<a href="cancel-orders.php" class="btn btn-danger">Back</a>
			<?php } ?>
            </div>
		  ...
		  $alltotal=''; $tot='';$qty='';$price=''; while($items=mysql_fetch_array($orders_itmes)){  
		   ...
		   <?php  $cancelation_dates=mysql_query("select ocid,cancelation_reason,comment from orders_cancelation where oid='".$orders['oid']."'");
						$ca=mysql_fetch_array($cancelation_dates);
						 } ?>
		   <tr><td></td><td></td><td>Total Qty:&nbsp;<?php echo $qty; ?></td><td>Total Price  Rs. <?php echo  $price; ?></td><td>Total Subprice <?php echo $alltotal; ?></td></tr>
			 ...
			 <div class="row">
			 <div class="col-md-12">
			 <h3>Cancelation Details</h3>
			 <p><strong>Cancelation Reason:&nbsp;&nbsp;</strong><?php echo $ca['cancelation_reason'];  ?></p>
			  <p><strong>Cancelation Description:&nbsp;&nbsp;</strong><?php echo $ca['comment'];  ?></p>
			 </div>
			 </div>
			</div>
...
<input type="submit" class="btn btn-primary" value="Search" onclick="return search_result()" />
		...
		<div class="table-responsive">
		    <table id="datatable" class="table table-striped table-bordered">
					  ...
					  <?php 
						...
						$cancelation_dates=mysql_query("select ocid,cancelation_reason from orders_cancelation where oid='".$row['oid']."'");
						$ca=mysql_fetch_array($cancelation_dates);
                        ...
                        <tr>
                          ...
                          <td><?php echo $ca['cancelation_reason']; ?></td>
						  <td><?php echo $row['date']; ?></td>
						   <td>
						  
						   <a href="#" class="btn btn-success" title="Cancel Orders"><?php echo $row['cancelation_date']; ?></a>
						
						  </td>
                          <td>
						   <?php if(isset($_GET['searchkeyowords'])) { ?>
						  <a href="cancel-orders.php?vieworders=opendata&catid=<?php echo $row['order_id']; ?>&searchkeyowords=<?php echo $_GET['searchkeyowords']; ?>"  class="btn btn-primary" title="Edit"><span class="glyphicon glyphicon-eye-open"></span></a>&nbsp;&nbsp;
						  <?php }else{ ?>
						    <a href="cancel-orders.php?vieworders=opendata&catid=<?php echo $row['order_id']; ?>"  class="btn btn-primary" title="Edit"><span class="glyphicon glyphicon-eye-open"></span></a>&nbsp;&nbsp;
						  <?php } ?>
						</td>
                        </tr>
						<?php } }else{ ?>
		...
		<div class="row">
				<div class="col-md-12">
				<?php if(isset($_GET['searchkeyowords'])) { echo cancelsearching($setLimit,$page,$_GET['searchkeyowords']); }else { echo canelorders($setLimit,$page); } ?>
		</div>
			</div>
		</div>
		</div>
...
window.location='cancel-orders.php?searchkeyowords='+search_id2;
return false;
}
